...
export default class DXGameMgr extends GameMgr {
    ...
    @ConditionFilter(ErrorCode.UNEXCEPT_OPERATE, DXOperate.WAIVE)
    C_Waive(gamber: GamberModel) {
        this.betting(<DXGamberModel>gamber, DXOperate.WAIVE, 0);
    }
    ...
    C_SeeCard(gamber: GamberModel) {
        if (!this.rubCards[gamber.userId]) {
            this.rubCards[gamber.userId] = true;
            let cardType = DXCardPointMgr.getCardType(gamber.holds);
    ...
    otherAllEliminate(gamber: GamberModel) {
        for (let otherGamber of this.gambers) {
            if (gamber == otherGamber) {
                continue;
            }
            if (!otherGamber.eliminate) {
                return false;
            }
        }
        return true;
    ...
    getOptionalOperate(gambler: GamberModel) {
        if (this.isReverseBeltMode && gambler.scoreBetting > 0) {
            return DXOptionalOperate.REVERSE_BELT;
        }
        if (this.isBlintEatMode) {
            if (this.bankerOnlyEat) {
                if (this.banker == gambler) {
